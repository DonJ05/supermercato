...
if [ ! -s ./log/status_offline.log ];then #controllo se il file status_offline.log è vuoto o non esiste, in tal caso lo creo e aggiungo l'intestazione
	echo "Data;Ora_Offline;Ora_Online;Durata_Secondi" > ./log/status_offline.log
fi

if grep -q -E "^$1" ./log/status_offline.log;then #controllo se il giorno è già presente nel file status_offline.log
    echo "ATTENZIONE: il giorno $1 è già presente in status_offline.log"
    exit 1
fi


# Estrazione dei dati dal file di log e calcolo della durata offline
date="$1"
awk -F';' -v date="$date" ' 
