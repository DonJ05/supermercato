...
if [ -z  "$count" ];then
	...
	awk -F',' 'i[$0]==0 {i[$0]++; print}' $file > tmm && mv tmm $file #elimina le righe duplicate, mantenendo solo la prima occorrenza
sleep 1
...
if ! grep -q "server_online;online;$file;.*;OK" "$LOG"; then #controlla se il file è già stato inviato al server
