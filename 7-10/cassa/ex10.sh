...
cassa_time=$(date +%s)  #converto l'orario della cassa in orario unix
...
if ! [[ "$server_time" =~ ^[0-9]+$ ]];then
    echo "errore nella richiesta dell'orario al server"
	cat error.log 
	rm error.log
	exit 1
else
...
dif=${dif#-} #toglie il segno "-" se presente, in questo modo ottengo sempre un numero positivo
if [ $dif -gt 5 ];then
...
diff=${diff#-}
if [ "$diff" -le $s ];then #se la differenza tra l'orario del server e quello della cassa è minore o uguale alla tolleranza, allora l'orario è sincronizzato
